...
#[tokio::test]
async fn test_quic_transfer_functional() -> Result<(), anyhow::Error> {
    // Run in both debug and release to verify QUIC protocol correctness
    let size = 10 * 1024; // 10KB
    let (src_path, dest_dir) = create_temp_file("quic_func", size)?;

    let trust_path = std::env::temp_dir().join("waft_perf_trust_quic_func");
    if trust_path.exists() {
        let _ = fs::remove_file(&trust_path);
    }
    let trust_store = Arc::new(TrustStore::load_or_create(&trust_path)?);
    ...
    // Use a dynamically allocated UDP port
    let bind_addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 0);
    // Find an open UDP port by binding a standard socket temporarily
    let socket = std::net::UdpSocket::bind(bind_addr)?;
    let local_addr = socket.local_addr()?;
    drop(socket);

    // Initialize crypto provider just in case (tests run in isolation)
    waft::quic_transport::init_crypto_provider();

    let trust = Arc::clone(&trust_store);
    let dest = dest_dir.clone();
    let receiver_handle = tokio::spawn(async move {
        let _ = waft::quic_transfer::start_quic_receiver(local_addr, trust, dest).await;
    });

    // Wait a short duration for the UDP socket to bind
    tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;

    waft::quic_send::send_file_quic(local_addr, &sender_identity, &src_path, None).await?;

    // Verify file exists and has correct size
    let dest_file_path = dest_dir.join("waft_perf_src_quic_func");
    assert!(dest_file_path.exists());
    assert_eq!(fs::metadata(&dest_file_path)?.len(), size as u64);
...
#[tokio::test]
async fn perf_quic_1mb_under_100ms() -> Result<(), anyhow::Error> {
    if cfg!(debug_assertions) {
        println!("Skipping performance test in debug mode");
        return Ok(());
    }
    let size = 1024 * 1024; // 1MB
    let (src_path, dest_dir) = create_temp_file("quic_1mb", size)?;

    let trust_path = std::env::temp_dir().join("waft_perf_trust_quic_1mb");
    if trust_path.exists() {
        let _ = fs::remove_file(&trust_path);
    }
    let trust_store = Arc::new(TrustStore::load_or_create(&trust_path)?);
    ...
    println!("QUIC 1MB Loopback Roundtrip: {:?}", elapsed);

    // Verify file exists
    let dest_file_path = dest_dir.join("waft_perf_src_quic_1mb");
    assert!(dest_file_path.exists());
    assert_eq!(fs::metadata(&dest_file_path)?.len(), size as u64);
    ...
    assert!(
        elapsed < limit,
        "QUIC 1MB transfer took too long: elapsed={:?}, limit={:?}",
        elapsed,
        limit
    );
...
#[tokio::test]
async fn perf_quic_100mb_throughput_floor() -> Result<(), anyhow::Error> {
    if cfg!(debug_assertions) {
        println!("Skipping performance test in debug mode");
        return Ok(());
    }
    let size = 100 * 1024 * 1024; // 100MB
    let (src_path, dest_dir) = create_temp_file("quic_100mb", size)?;

    let trust_path = std::env::temp_dir().join("waft_perf_trust_quic_100mb");
    if trust_path.exists() {
        let _ = fs::remove_file(&trust_path);
    }
    let trust_store = Arc::new(TrustStore::load_or_create(&trust_path)?);
    ...
    let mb_per_sec = (size as f64 / (1024.0 * 1024.0)) / secs;
    println!(
        "QUIC 100MB Throughput: {:.2} MB/s (elapsed: {:.2}s)",
        mb_per_sec, secs
    );

    // Verify file exists
    let dest_file_path = dest_dir.join("waft_perf_src_quic_100mb");
    assert!(dest_file_path.exists());
    assert_eq!(fs::metadata(&dest_file_path)?.len(), size as u64);
    ...
    assert!(
        mb_per_sec >= floor,
        "QUIC 100MB throughput below floor: {:.2} MB/s (floor: {:.2} MB/s)",
        mb_per_sec,
        floor
    );
